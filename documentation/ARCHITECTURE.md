...
- **Rate limiting:** global per‑IP + per‑user fixed windows, plus stricter sub‑limits on the expensive or spam‑prone paths (natural roll, data export, feedback); tunable via env. All of them share one fixed‑window primitive, held in process memory by default — a deliberate choice, not an oversight: on serverless each instance keeps its own counters, so the effective budget is N× the configured one. Accepted deliberately at current scale: a shared store would put a round‑trip in front of the request to tighten budgets that exist as defence‑in‑depth, and the one limiter guarding real money (natural roll → Gemini) protects spend that is currently negligible. Revisit when the natural‑roll bill or the traffic makes an N× budget worth a round‑trip; the counter is one class with one method, so the swap stays a single central change.
