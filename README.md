...
The full writeup is in [`docs/CASE_STUDY.md`](./docs/CASE_STUDY.md).
...
docs/            architecture, case study, algorithms, API reference, runbooks
```
...
Sanity check: `http://localhost:4000/health` returns `{ ok: true, db: "up" }`. Every environment variable is validated at boot — the server refuses to start on a bad value. The full table is in [`docs/SETUP.md`](./docs/SETUP.md).

Note on data: the raw award Excel files are private project assets and are not in the repository. A fresh clone runs against whatever your database is seeded with; the pipeline that builds the dataset is documented in [`docs/DATA-WORKFLOW.md`](./docs/DATA-WORKFLOW.md).
...
[`DOCUMENTATION.md`](./DOCUMENTATION.md) is the map of everything — which document to read first depending on whether you are reviewing the engineering, joining the codebase, or running a data build.
