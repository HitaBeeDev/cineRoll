...
> **TL;DR for reviewers.** The genuinely hard part isn't the UI or CRUD — it's the **entity resolution** that turns ~hundreds of messy, differently‑titled award‑nomination rows into one clean film catalog, and the **content‑based recommendation system** built on the signals collected on top of it. Architecture details: [`ARCHITECTURE.md`](./ARCHITECTURE.md).

---

## The headline problem — one film, five award bodies, a hundred years

Award data is deceptively messy. The source is hand‑curated Excel — **one row per nomination** — across the Oscars, Golden Globes, Cannes, the Berlinale, and the IMDB Top 250. A single film shows up many times:
...
**The roll** (`lib/random.ts`) — the product's identity stays pure random by default. The opt‑in "Roll from my taste" samples by **softmax over taste score with ε‑greedy exploration**, biasing toward your taste while guaranteeing every film keeps non‑zero probability — an explicit exploration/exploitation tradeoff, not a `WHERE` clause in disguise.
...
- **Grounding an LLM feature.** "Describe It" (`lib/naturalRoll.ts`) maps free text → filters via Gemini, but the model only *interprets* — the **database decides** which films exist. Model output is validated against DB‑derived allowed values, near‑misses are remapped ("Sci‑Fi" → "Science Fiction"), invalid values dropped, with a relax‑fallback when nothing matches. The LLM never invents a film.
...
*See also: [`ARCHITECTURE.md`](./ARCHITECTURE.md) · [`RECOMMENDATIONS.md`](./RECOMMENDATIONS.md) · [`DECISIONS.md`](./DECISIONS.md) · [`docs/DATA-WORKFLOW.md`](./docs/DATA-WORKFLOW.md)*
