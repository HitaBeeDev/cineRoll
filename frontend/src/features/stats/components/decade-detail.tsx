  ...
  return (
    <div className="flex flex-col rounded-xl border border-white/10 bg-[#0c0c14] p-6">
      ...
      <div className="mt-1 flex items-center gap-3">
        ...
        {decade.decade === peakDecade && <span className="rounded-full border border-accent/40 bg-accent/10 px-2.5 py-1 font-[family-name:var(--font-geist-mono)] text-xs uppercase tracking-[0.16em] text-accent-soft">Peak era</span>}
      </div>
      <div className="mt-6 flex items-end gap-3">
        ...
        <span className={cn("mb-0.5 ml-auto rounded-md px-2 py-1 font-[family-name:var(--font-geist-mono)] text-xs font-medium tabular-nums", versusAverage >= 0 ? "bg-accent/12 text-[#ff8a83]" : "bg-white/[0.06] text-[#9e9ab0]")}>{versusAverage >= 0 ? "+" : ""}{versusAverage.toFixed(0)}% vs avg decade</span>
      </div>
