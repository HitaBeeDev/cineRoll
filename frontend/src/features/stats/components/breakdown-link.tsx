...
export function BreakdownLink({ label, count, color, href, percent }: BreakdownLinkProps) {
  return (
    <Link href={href} className="group rounded-md border border-white/10 bg-ink-850 p-4 transition-colors hover:border-white/20 hover:bg-[#12121c]">
      ...
      <p className="mt-4 font-[family-name:var(--font-display)] text-3xl font-bold leading-none text-[#f4f0f7]">{count.toLocaleString()}</p>
