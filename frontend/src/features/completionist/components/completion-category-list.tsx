    ...
    <div className="mt-7 grid gap-x-8 gap-y-2 sm:grid-cols-2">
      {categories.map((category, index) => (
        <Link
          ...
          className="group -mx-3 rounded-lg px-3 py-3 transition-colors hover:bg-[#0d0d1a] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
          aria-label={`Browse ${category.label}: ${category.watched} of ${category.total} watched`}
        >
          <div className="flex items-baseline justify-between gap-3">
            <span className="truncate font-[family-name:var(--font-geist-mono)] text-[13px] font-semibold text-[#c8c8d2] transition-colors group-hover:text-fg-hi">
              {category.label}
            </span>
            <span className="shrink-0 font-[family-name:var(--font-geist-mono)] text-[12px] tabular-nums text-[#9a9aac]">
              ...
              <span className="ml-2 text-accent" aria-hidden>→</span>
            </span>
          </div>
