...
export function TasteCard({
  ...
  return (
    <button
      ...
      className={cn(
        "group relative overflow-hidden border bg-ink-900 text-left shadow-[0_22px_70px_rgba(0,0,0,0.34)] transition duration-200",
        ...
        selected
          ? "border-accent shadow-[0_0_0_1px_rgba(232,69,60,0.28),0_24px_80px_rgba(232,69,60,0.2)]"
    ...
    >
      {film.posterUrl ? (
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-ink-800 p-4 text-center">
          <span className="font-[family-name:var(--font-display)] text-lg font-semibold leading-tight text-fg-hi">
            {film.title}
          </span>
        </div>
      )}
      <div
        className={cn(
          ...
          selected ? "bg-ink-900/30" : "bg-transparent",
        )}
      />
