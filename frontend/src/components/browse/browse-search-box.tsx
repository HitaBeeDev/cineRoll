...
export function BrowseSearchBox({
  ...
  return (
    <div ref={containerRef} className="relative w-full min-w-0 sm:grow sm:basis-[180px]">
      <Search className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-edge-hover" />
      <input
        type="text"
        role="combobox"
        ...
        className="h-10 w-full rounded-md border border-white/10 bg-white/[0.045] pl-9 pr-3 text-[13px] text-fg-hi outline-none transition-colors placeholder:text-fg-faint hover:border-white/18 focus:border-accent/70 focus:ring-2 focus:ring-accent/15"
      />
      {open && results && hasSuggestions && (
        <div
          id={listboxId}
          role="listbox"
          className="absolute left-0 top-full z-50 mt-1 w-full overflow-hidden rounded-lg border border-white/12 bg-ink-850 shadow-[0_16px_48px_rgba(0,0,0,0.6)]"
        >
          {results.films.length > 0 && (
            <>
              <div className="px-3 pt-2.5 pb-1 font-[family-name:var(--font-geist-mono)] text-[11px] uppercase tracking-[0.28em] text-edge-hover">
                Films
              </div>
              {results.films.map((film, i) => (
                ...
                >
                  <Clapperboard className="h-3 w-3 shrink-0 text-edge-hover" aria-hidden />
                  <span className="min-w-0 flex-1 truncate text-[13px] text-fg">
                    {film.title}
                  </span>
                  <span className="shrink-0 font-[family-name:var(--font-geist-mono)] text-[11px] text-edge-hover">
                    {formatFilmYear(film)}
                  </span>
                </button>
              ))}
          ...
          {results.people.length > 0 && (
            <>
              <div className={cn(
                "px-3 pb-1 font-[family-name:var(--font-geist-mono)] text-[11px] uppercase tracking-[0.28em] text-edge-hover",
              ...
              {results.people.map((person, j) => {
                ...
                return (
                  ...
                  >
                    <Search className="h-3 w-3 shrink-0 text-edge-hover" aria-hidden />
                    <span className="min-w-0 flex-1 truncate text-[13px] text-fg">
                      {person.name}
                    </span>
                    <span className="shrink-0 font-[family-name:var(--font-geist-mono)] text-[11px] capitalize text-edge-hover">
                      {person.roles.join(" · ")}
                    </span>
                  </button>
                );
